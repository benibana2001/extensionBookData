// node_modules/benibana_bookdata/dist/main.js
let OpenBD = class {
  _isbn = '';
  ...
  message = {
    NO_BOOK:
      '\u6307\u5B9A\u306EISBN\u306B\u8A72\u5F53\u3059\u308B\u60C5\u5831\u304C\u30C7\u30FC\u30BF\u30D9\u30FC\u30B9\u306B\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F.'
  };
...
let DEFAULT_LIB_REQUEST = {
  appkey: '',
  isbn: '',
  systemid: '',
  pollingDuration: 2e3
};
let DEFAULT_LIB_RESPONSE = {
  libkey: [],
  reserveurl: '',
  continue: 0,
  books: {}
...
let BeniBook = class {
  ...
  async searchBookCoverURL(isbn2) {
    return (await this._openbd.search(isbn2)).coverurl;
  }
  async searchBookTitle(isbn2) {
    return (await this._openbd.search(isbn2)).title;
  }
  async searchLibraryStock(request) {
    return await this._calil.search(request);
  }
};
let beniBook = new BeniBook();

// src/main.ts
let isbn = '9784837987499';
console.log(`bookCover: ${await beniBook.searchBookCoverURL(isbn)}`);
console.log(`bookTItle: ${await beniBook.searchBookTitle(isbn)}`);
let CALIL_KEY = '46a2412f4ceb07b72a251150f2533c74';
let systemid = 'Tokyo_Setagaya';
let pollingDuration = 500;
console.log(
